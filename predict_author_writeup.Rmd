...
output: pdf_document
abstract: We employ both supervised and unsupervised learning methods to predict the authorship of philosophy text across four authors. For our supervised model, we use multinomial lasso regression using the tokens (words) as features and a line of text as the outcome. We get an accuracy of 67 percent with this technique. The unsupervised model uses topic modeling to give the probability that a document belong to a particular author. Based on the results, it seem to perform just as well as the supervised learning method as they both fail to correctly attribute works to Plato.
---
